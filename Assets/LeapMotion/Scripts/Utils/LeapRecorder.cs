...
public class LeapRecorder {
  ...
  public string SaveToNewFile() {
        ...
        string path = "./Assets/default.bytes";
  ...
  public string SaveFrameToNewFile(string path)
    {
        if (File.Exists(@path))
        {
            File.Delete(@path);
        ...
        FileStream stream = new FileStream(path, FileMode.Append, FileAccess.Write);
        byte[] frame_size = new byte[4];
        frame_size = System.BitConverter.GetBytes(frames_[1].Length);
        stream.Write(frame_size, 0, frame_size.Length);
        stream.Write(frames_[1], 0, frames_[1].Length);
        stream.Close();
        return path;
    }
